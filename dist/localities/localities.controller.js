"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LocalitiesController = void 0;
const common_1 = require("@nestjs/common");
const jwt_auth_guard_1 = require("../auth/jwt-auth.guard");
const localities_service_1 = require("./localities.service");
let LocalitiesController = class LocalitiesController {
    constructor(_localitiesService) {
        this._localitiesService = _localitiesService;
    }
    create(area) {
        return this._localitiesService.create(area);
    }
    readAll() {
        return this._localitiesService.readAll();
    }
    readByIsoCode(countryCode, areaCode, isoCode) {
        return this._localitiesService.readByIsoCode(countryCode, areaCode, isoCode);
    }
    update(countryCode, areaCode, isoCode, locality) {
        return this._localitiesService.update(countryCode, areaCode, isoCode, locality);
    }
    delete(countryCode, areaCode, isoCode) {
        return this._localitiesService.delete(countryCode, areaCode, isoCode);
    }
};
__decorate([
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.Post)('/create'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], LocalitiesController.prototype, "create", null);
__decorate([
    (0, common_1.Get)('/read/all'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], LocalitiesController.prototype, "readAll", null);
__decorate([
    (0, common_1.Get)('/read/:countryCode/:areaCode/:localityCode'),
    __param(0, (0, common_1.Param)('countryCode')),
    __param(1, (0, common_1.Param)('areaCode')),
    __param(2, (0, common_1.Param)('localityCode')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object, Object]),
    __metadata("design:returntype", void 0)
], LocalitiesController.prototype, "readByIsoCode", null);
__decorate([
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.Patch)('/update/:countryCode/:areaCode/:localityCode'),
    __param(0, (0, common_1.Param)('countryCode')),
    __param(1, (0, common_1.Param)('areaCode')),
    __param(2, (0, common_1.Param)('localityCode')),
    __param(3, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object, Object, Object]),
    __metadata("design:returntype", void 0)
], LocalitiesController.prototype, "update", null);
__decorate([
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.Delete)('/delete/:countryCode/:areaCode/:localityCode'),
    __param(0, (0, common_1.Param)('countryCode')),
    __param(1, (0, common_1.Param)('areaCode')),
    __param(2, (0, common_1.Param)('localityCode')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object, Object]),
    __metadata("design:returntype", void 0)
], LocalitiesController.prototype, "delete", null);
LocalitiesController = __decorate([
    (0, common_1.Controller)('api/localities'),
    __metadata("design:paramtypes", [localities_service_1.LocalitiesService])
], LocalitiesController);
exports.LocalitiesController = LocalitiesController;
//# sourceMappingURL=localities.controller.js.map